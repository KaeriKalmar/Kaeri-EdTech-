// ============================================================
// === KAERI EDTECH QUIZ ENGINE - HYBRID MASTER (v11.6 DUAL-MODE FINAL) ===
...
// ============================================================
// === 6. QUIZ ENGINE (MCQ, SHORT ANSWER, ESSAY) ===
// ============================================================
...
// ============================================================
// === 8. FLASHCARD ENGINE - DUAL MODE WITH BRAND CONSISTENCY ===
// ============================================================

// --- TOPIC SELECTION (Brand 1 style grid) ---
function renderFlashcardTopics() {
    ...
    // Brand 1 style topic grid (purple theme)
    const header = document.createElement("h2");
    header.innerText = "Select Flashcard Topic";
    header.style.cssText = "text-align:center; margin-bottom:25px; color:#fff;";
    container.appendChild(header);

    const listDiv = document.createElement('div');
    listDiv.style.cssText = "display:grid; grid-template-columns:repeat(auto-fill, minmax(200px,1fr)); gap:15px;";
    ...
    for (const topic in currentFlashcardTopics) {
        ...
        btn.style.cssText = `
            ...
            transition:0.3s;
        `;
        btn.onmouseover = () => { btn.style.background = '#3e506e'; };
        btn.onmouseout = () => { btn.style.background = '#2b3a55'; };
...
// --- MODE SELECTION (Brand-specific visual identity) ---
...
function showFlashcardModeSelection(topic) {
    ...
    // Render Choice Menu with brand colors
    container.innerHTML = `
        <div style="text-align: center; animation: fadeIn 0.3s;">
            ...
            <div style="display: flex; flex-direction: column; gap: 15px; max-width: 450px; margin: 0 auto;">
                ...
                <!-- LINEAR MODE (Brand 1 - Purple) -->
                <button onclick="startFlashcards('${topic}', 'linear')" 
                    style="background: #1e2a3a; border: 2px solid #6f42c1; padding: 20px; border-radius: 12px; text-align: left; cursor: pointer; transition: 0.2s; box-shadow: 0 4px 6px rgba(0,0,0,0.3);">
                    <div style="display: flex; align-items: center; gap: 15px;">
                        <span style="font-size: 2em;">📖</span>
                        <div>
                            <div style="font-size: 1.2em; color: white; font-weight: bold;">Standard Review</div>
                            <div style="font-size: 0.9em; color: #a0a8b4; margin-top: 5px;">Review all <strong>${totalCards}</strong> cards in order. Perfect for cramming.</div>
                        </div>
                    </div>
                </button>

                <!-- SRS MODE (Brand 2 - Green/Yellow) -->
                <button onclick="startFlashcards('${topic}', 'srs')" 
                    style="background: #1e2a3a; border: 2px solid #28a745; padding: 20px; border-radius: 12px; text-align: left; cursor: pointer; transition: 0.2s; box-shadow: 0 4px 6px rgba(0,0,0,0.3);">
                    <div style="display: flex; align-items: center; gap: 15px;">
                        <span style="font-size: 2em;">🧠</span>
                        <div style="flex: 1;">
                            ...
                            <div style="font-size: 0.9em; color: #a0a8b4; margin-top: 5px;">Algorithm-based. Focus on what you're about to forget.</div>
                        </div>
                    </div>
                </button>
        ...
        if (srsQueue.length === 0) {
            ...
            <div style="text-align: center; animation: fadeIn 0.5s;">
                ...
                <div style="margin-top: 20px; display: flex; flex-direction: column; gap: 10px; align-items: center;">
                    <button class="restart-button" onclick="startFlashcards('${topic}', 'linear')" style="background:#6f42c1;">📖 Switch to Standard Review</button>
...
// --- DISPLAY ENGINE (Brand-Consistent) ---
function displayFlashcard() {
    ...
    // Smart Layout Analyzer (used only for SRS mode)
    ...
    const isLinear = currentFlashcardMode === 'linear';
    const theme = {
        primary: isLinear ? '#6f42c1' : '#28a745',
        bg: isLinear ? '#2b3a55' : '#1a1a2e',
        headerColor: isLinear ? '#fff' : '#72efdd',
        cardMinHeight: isLinear ? '250px' : '300px',
        cardPadding: isLinear ? '30px' : '40px',
        fontSize: isLinear ? '1.2em' : '1.4em'
    };

    const modeLabel = isLinear ? "📖 Standard Review" : "🧠 SRS Study";
    const frontLayout = isLinear ? '' : getLayoutClass(cardObj.front);
    const backLayout = isLinear ? '' : getLayoutClass(cardObj.back);
    ...
    let html = `
        ...
        <div class="flashcard-wrapper" style="max-width: ${isLinear ? '600px' : '800px'}; margin:20px auto; perspective:1000px;">
            <div class="flashcard ${isCardFront ? '' : 'back-active'}" 
                 onclick="flipCard()"
                 style="position:relative; width:100%; min-height:${theme.cardMinHeight}; padding:${theme.cardPadding}; 
                        background:${theme.bg};
                        border-radius:15px;
                ...
                <div class="card-face card-front" 
                    ...
                    ${isLinear ? '' : '<div style="margin-top:20px; font-size:0.75em; color:#8892b0; font-style:italic; opacity:0.8;">(Tap to flip)</div>'}
                </div>
    ...
    // --- CONTROLS SECTION (Brand-Consistent) ---
    html += `<div class="flashcard-nav-buttons" style="margin-top: 25px;">`;

    if (isLinear) {
        // LINEAR CONTROLS (Brand 1: Prev/Flip/Next)
        ...
        html += `
            <div style="display:flex; gap:10px; justify-content:center;">
                <button onclick="prevFlashcard()" ${currentCardIndex === 0 ? 'disabled' : ''} 
                    style="background:#3e506e; color:white; padding:10px 20px; border:none; border-radius:5px; cursor:pointer;">⬅️ Prev</button>
                <button onclick="flipCard()" 
                    style="background:${theme.primary}; color:white; padding:10px 20px; border:none; border-radius:5px; cursor:pointer; font-weight:bold;">🔄 Flip Card</button>
                <button onclick="nextFlashcard()" 
                    style="background:${isLast ? '#28a745' : '#3e506e'}; color:white; padding:10px 20px; border:none; border-radius:5px; cursor:pointer;">
                    ${isLast ? 'Finish 🏁' : 'Next ➡️'}
                </button>
            </div>
        `;
    } else {
        // SRS CONTROLS (Brand 2: Show / Rate)
        if (isCardFront) {
            html += `<button onclick="flipCard()" style="width:100%; background:#007bff; color:white; padding:15px; border:none; border-radius:8px; font-weight:bold; cursor:pointer; box-shadow:0 4px 0 #0056b3;">🔄 Show Answer</button>`;
        } else {
            html += `
                <div style="display:grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap:10px; width:100%;">
                    <button onclick="rateCard(0)" style="background:#dc3545; color:white; font-size:0.8em; padding:12px 2px; border:none; border-radius:6px; cursor:pointer;">❌ Again<br><small style="opacity:0.8">1d</small></button>
                    <button onclick="rateCard(3)" style="background:#ffc107; color:#333; font-size:0.8em; padding:12px 2px; border:none; border-radius:6px; cursor:pointer;">😬 Hard<br><small style="opacity:0.8">2d</small></button>
                    <button onclick="rateCard(4)" style="background:#28a745; color:white; font-size:0.8em; padding:12px 2px; border:none; border-radius:6px; cursor:pointer;">✅ Good<br><small style="opacity:0.8">4d</small></button>
                    <button onclick="rateCard(5)" style="background:#17a2b8; color:white; font-size:0.8em; padding:12px 2px; border:none; border-radius:6px; cursor:pointer;">🚀 Easy<br><small style="opacity:0.8">7d</small></button>
                </div>
            `;
        }
    }
    ...
    // Back button (brand-neutral)
    html += `<button class="back-to-topics-button" style="margin-top: 20px; background: transparent; color: #888; border:1px solid #3e506e; padding:10px 20px; border-radius:20px; cursor:pointer;" onclick="renderFlashcardTopics()">⬅️ Back to Topics</button>`;
...
// --- COMPLETION SCREEN (Brand-Consistent) ---
function showFlashcardCompletion() {
    ...
    // Buttons with brand-specific colors
    const restartBtn = document.createElement("button");
    restartBtn.innerText = "🔁 Review Again";
    restartBtn.className = "restart-button";
    restartBtn.style.marginRight = "10px";
    restartBtn.style.background = isLinear ? '#6f42c1' : '#28a745';
...
// ============================================================
// === MODIFIED: Quiz Renderers with Tiered Access ===
// ============================================================
...
// ============================================================
// === 6. QUIZ ENGINE (Unchanged - Works with new starters) ===
// ============================================================
